...
/**
 * 요구사항 -> AOP를 사용하여 공통 기능을 처리 또는 횡단 관심사를 처리
 * spring08-AOP-basic 에서 AOP를 사용하지 않은 예와 비교해본다
 * 
 * 요구사항 -> AOP로 처리할 공통 또는 횡단관심사항( Cross Cutting Concern )
 ...
 * 2. 이후 공통 작업 (Cross Cutting Consern 횡단 관심사 or 공통 관심사)의 업무가 변경되어도
 * 	  여러 곳을 수정 작업 필요 없고 오직 AOP 모듈만 업데이트 하면 된다 
 * 	  높은 응집도 high cohesion : Core 은 Core에만 집중, Cross cutting 은 별도로 전담
 * 	  낮은 결합도 loose coupling : Cross Cutting이 변경되어도 Core에는 영향이 없음 
 * 
 */
...
public class Spring09AopSolutionApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		//아래는 Sping Container 가 주입해준 Proxy(CGLIB) 대리인 객체
