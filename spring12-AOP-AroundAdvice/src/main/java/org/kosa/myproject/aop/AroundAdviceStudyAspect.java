...
public class AroundAdviceStudyAspect {
	 ...
	 * 관계없이 항상 수행) after-returning: 메서드 정상 실행 후 값이 리턴될 때 after-throwing: Exception 및
	 * Error 발생될 때
	 */
	...
	public Object testAround(ProceedingJoinPoint point) throws Throwable {
		// 실제 Core를 수행, 만약 Around Advice 환경에서 반드시 명시해야 한다.
		// 아래 코드를 명시하지않으면 Core 가 실행되지 않는다
		...
		try {
			...
			if (retValue != null) {// 리턴값이 있을 때
				...
				// retValue+=" 불목? ";
				logger.debug("**AOP After-Returing**return value:{}", retValue);// 정상적으로 리턴할 떄 실행하는 advice
			}
		} catch (InventoryException ie) {// after throwing: 재고부족으로 예외 발생할 때 대처
			// 공급처에 정보 전달
			logger.error("**AOP After-Throwing**", ie.getMessage(), ie);
			throw ie;
		} finally {
