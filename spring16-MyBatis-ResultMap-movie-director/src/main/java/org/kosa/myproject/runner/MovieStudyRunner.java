...
//		System.out.println("총 영화수:"+movieCount);
		...
		// 2. 영화(감독정보포함)리스트 조회: 상세 영ㅎㅘ 게시물 조회와는 다르게 리스트 상에 필요한 정보만 부분적으로 조회
//		List<Movie> list = movieMapper.findMovieList();
//		for (Movie movie : list) {
//			// Movie has a Director 정보가 잘 조회되는 지를 테스트
//			System.out.println(movie.getMovieId() + "" + movie.getTitle() + "" + movie.getDirector().getDirectorName()
//					+ "" + movie.getDirector().getDirectorId());
//		}
		// 3. 영화 상세정보조회(영화 정보와 감독 정보를 상세히 보여주기 위함)
//		Long movieId=1L;
...
//		System.out.println(detailMovie);
		...
		Movie movie = new Movie("살인의 추억","스릴러",1000L,director);
