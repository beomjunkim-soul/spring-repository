...
@Controller
public class MyTestController {// client가 get 방식으로 요청시 찾는 url pattern 지정
	@GetMapping("/hello")
	public String hello(Model model) {// Model: 클라이언트에게 응답할 정보를공유하기 위한 객체
		// view html 에 정보 공유 : request.setAttribute("info","떡볶이");
		model.addAttribute("info", "떡볶이");
		// forward 방식 : ViewResolver 에 의해 src/main/resources/template/result1.html
	...
	public String paramTest(String customerId, int customerAge, Model model) {// HandlerAdapter 가 관여. 매개변수를 호출
	...
	/**
	 * client에서 post 방식으로 요청하여 @PostMapping 으로 처리 client 의 form에서 동일한 name( menu )
	 * 으로 여러 개의 value 를 전송한다 이 경우 Servlet/JSP 에서는 request.getParameterValues():
	 * String[] 으로 요청을 처리했음 Spring에서는 이것을 HandlerAdapter 가 요청 데이터에 대한 배열 객체를 생성해서
	 * 우리의 컨트롤러 메서드에 전달해준다
	 */
	...
	public String paramTest2(String[] menu) {
		...
		// return "redirect:result3"; -> 안됨
		return "redirect:/result3-view";
		// Spring 환경에서 redirect는 FrontController 인 DispatcherServlet으로 가게 되어 있고 적절한
		// 컨트롤러가 있을 때 응답할 수 있다
	}
	...
	public String customerShowCustomerList(Model model) {
