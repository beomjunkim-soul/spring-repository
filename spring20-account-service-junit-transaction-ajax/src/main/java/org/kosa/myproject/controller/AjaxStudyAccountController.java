...
@Controller
@RequestMapping("/api/accounts")//api/복수형 명사
public class AjaxStudyAccountController {
	...
	@GetMapping("/{accountNumber}") // /api/accounts/1 ==> 1 계좌번호의 계좌조회
	@ResponseBody//@ResponseBody 와 Jackson 라이브러리가 동작되어 JSON으로 자동 변환되어 응답
	public Account getAccountByNumber(@PathVariable Long accountNumber) {//@PathVariable 로 경로의 값을 매개변수에 할당받을 수 있음
